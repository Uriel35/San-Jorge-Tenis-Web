...
window.addEventListener('DOMContentLoaded', async (e) => {
    ...
    // const w = 800;
    // const h = 300;
    // const data = [{date: '15-01-2020',points: 5},{date: '15-02-2020',points: 10},{date: '15-03-2020',points: 15},{date: '15-04-2020',points: 20},{date: '15-05-2020',points: 25},{date: '15-06-2020',points: 30},{date: '15-07-2020',points: 35},{date: '15-08-2020',points: 40},{date: '15-09-2020',points: 45},{date: '15-10-2020',points: 50}
    // ]
    const padding = {
        vertical: 45,
        horizontal: 70
    ...
    let doublesCurrentUser = await requestFx(`http://localhost:3000/api/doubles/${currentUser._id}`, 'GET');
    // doublesCurrentUser = doublesCurrentUser.partner;
    if(doublesCurrentUser.partner) {
        let H2H = profileUser.matches.filter(match => {
            return match.rival == doublesCurrentUser.partner.name
        });
        if(H2H.length !== 0) {
    ...
    const RIVAL_TOOLTIP_NAME = document.getElementById('rival-name')
    const RIVAL_TOOLTIP_MATCHES = document.getElementById('rival-matches')
    const CURRENT_TOOLTIP_NAME = document.getElementById('current-name')
    const CURRENT__TOOLTIP_MATCHES = document.getElementById('current-matches')
    ALL_RIVALS.forEach(rival => {
        ...
        rival.addEventListener('mouseover', (e) => {
            RIVAL_TOOLTIP_NAME.innerText = rival.outerText;
            RIVAL_TOOLTIP_MATCHES.innerText = rivalH2H.filter(match => match.result == false).length
            CURRENT_TOOLTIP_NAME.innerText = profileUser.name;
            CURRENT__TOOLTIP_MATCHES.innerText = rivalH2H.filter(match => match.result == true).length
